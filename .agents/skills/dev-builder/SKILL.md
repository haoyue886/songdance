...
[开发规则清单]
    ...
    [Git 工作流]
        - 操作 Git 前核对根仓库、嵌套仓库、当前分支和目标远端归属；提交、推送只在实际拥有目标文件和远端的仓库执行
...
[Phase 完成度判断]
    所有 Task 完成后过四步走，全通过才算 Phase 完成：
        ...
        四、功能测试：启动 dev server 无错，新功能可用，现有功能未破坏；有 Playwright 测核心流程，无则 curl 查 API 返回再提醒用户看 UI。开发服务若包含 HMR、WebSocket、SSE 等开发态运行通道，必须按实际协议验证通道建立并完成一次事件或变更往返，不能只以页面 HTTP 成功判定服务正常
