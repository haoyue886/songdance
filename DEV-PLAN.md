...
- 仅在音频级延音证据或 MIDI CC64 存在时输出踏板标记；否则保留为不确定质量标记，不伪造演奏控制。
...
- Task 30.2–30.4 各自具备正例与反例，不能用 MIDI velocity 代替音频能量，不能用八度关系单独删除音符，不能给所有钢琴输入硬套中央 C 分手。
- 每个 Task 后重跑 16 段固定结构集并重置受影响产物的人工评级；`04-arpeggios` 必须由专业评审复评到至少“少量修改可用”，否则 Phase 30 不完成。
