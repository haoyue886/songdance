...
- Task 32.3 已完成代码与自动门禁；Task 32.5 的保守证据链和自动回归已完成，但 AC-074/078/082 仍受 `04-arpeggios` 结构目标与专业复评阻塞，不标记为完成。Task 32.1 已完成失败对照状态与当前结构修复但仍等待完整参考谱逐小节映射：K.545 保持 321 个原始模型音符、321 个清理后音符，左右手/未知事件为 102/218/1；`STAFF_DISTRIBUTION_SUSPECT` 通过，低音分配、低音小节覆盖和高音分配均为 1.0。MusicXML 使用 C 大调、局部调性感继续记录 G 大调、无假弱起。
...
- 10 段真人钢琴集与 16 段结构集已按严格 fail-closed 的 `harmonic-evidence-v4/96273e7507d9` 后处理重生成并绑定对应指纹：真人集指纹 `064c806fd52b652c7197890a934fe64dcaafa89996cdb2f00215f7b404002148`，结构集指纹 `7edcf80f120f5937646113d6950cafa33a1270ca6c6e6bc7d00c570f2c4ce821`。26 份 MusicXML 的 music21、`xmllint`、OSMD 共 78 项全部通过；两套人工评级均重置为 `pending`。离线包为 `songdance/api/dist/songdance-structure-review-7edcf80f120f.zip`，SHA-256 `851d597624940d13c88f28abae60496a00798f6f66cd07cd9ed5cc55e9f9bd20`，`unzip -t` 通过。最终自动门禁为 API `393 passed/6 skipped`、Web `140 passed`、TypeScript、ESLint、Ruff、Next.js 45 页面生产构建和 Playwright `26 passed`。E2E 使用高于用例数量的小时/日测试配额隔离跨用例计数，同时保留单客户端活动任务上限的 429 专项验证。
- 当前 `04-arpeggios` 证据：117 BPM、4/4、清理前 326/清理后 282、具备可测尾音证据的谐波候选移除 44，14 个稳定八音循环、119 个匹配槽位；`simple-arpeggio-filter-v7` 只删除 3 个具有完整能量、力度、时长和尾音证据的 harmonic pair，最终记谱音为 279。真实样本中原先 124 个 `same_pitch_decay` 候选均因缺少前音实际时间重叠而保守保留；合成正反例分别覆盖真实重叠衰减、久远历史同音、前音已结束、独立能量增长、力度比过高和时长比过高，每次获准删除均记录前后频带能量、能量比、力度比、时长比与重叠秒数。产物仍为单 Piano part、双谱表、每谱表每小节最多 2 个 voice、1 个 `mp`、1 个踏板区间（2 个 XML 端点），无未知声部；但 279 音、低音谱表最多 2 个 voice 明确不满足 AC-078/082 的 119 音与每谱表单主声部目标，必须继续作为 AC-074 专业复评阻塞项，不能声明人工通过。K.545 公共示例 provenance 为 `review_status=pending`、`review_state=review_paused`、恢复条件 `reference_aligned_transcription_ready`，最近完成评级 `needs_redo`。
