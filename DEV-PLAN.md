...
- Task 32.3、32.5 已完成代码与自动门禁，Task 32.1 已完成失败对照状态与当前结构修复但仍等待完整参考谱逐小节映射：K.545 保持 321 个原始模型音符、321 个清理后音符，左右手/未知事件为 102/218/1；`STAFF_DISTRIBUTION_SUSPECT` 通过，低音分配、低音小节覆盖和高音分配均为 1.0。MusicXML 使用 C 大调、局部调性感继续记录 G 大调、无假弱起。
...
- 10 段真人钢琴集与 16 段结构集已按 `harmonic-evidence-v3` 最终后处理版本重生成并绑定对应指纹：真人集指纹 `a9ef38ba097da77b61b087e77655f24cd5818c71910cc989af16c6790717a34`，结构集指纹 `95de400b3db94ee54896e9d5b26ddc654259fc63bfc8aee33f3088a319cb20a6`。26 份 MusicXML 的 music21、`xmllint`、OSMD 共 78 项全部通过；API 全量 `385 passed/6 skipped`，Web 单测 `140 passed`，TypeScript、ESLint、Ruff、Next.js 45 页面生产构建和 Playwright `26 passed` 全部通过。离线包为 `songdance/api/dist/songdance-structure-review-95de400b3db9.zip`，`unzip -t` 通过。
- 最终 `04-arpeggios` 证据：117 BPM、4/4、清理前 326/清理后 252、谐波候选移除 74，14 个稳定八音循环、119 个匹配槽位、琶音共振过滤 133 个，最终记谱音 119 个；单 Piano part、双谱表、1 个 `mp`、1 个踏板区间（2 个 XML 端点），无未知声部。K.545 公共示例 provenance 为 `review_status=pending`、`review_state=review_paused`、恢复条件 `reference_aligned_transcription_ready`，最近完成评级 `needs_redo`。
...
- Petzold 修正使既有固定集指纹按预期失效；重生成时发现 `04-arpeggios` 因通用泛音保护收紧而每周期多出一个 MIDI 79 假高音。`simple-arpeggio-filter-v6` 只在稳定重复周期、踏板覆盖与同一音符的起音/STFT 泛音对证据同时成立时删除候选，禁止跨周期复用同 velocity 证据；无音频证据时保留，并使用原始起音能量/力度一致性下界、时长归一化一致性区间与候选结束后的短窗频带尾音共同区分自然泛音和真实独立攻击。`harmonic-evidence-v3` 让通用 `extract→clean` 与简单琶音过滤共用同一保护逻辑，不再按泛音阶数硬编码：真实短八度或十二度结束后高频能量应明显下降，自然泛音则继续随尚未结束的低音存在；尾音比先扣除起音前稳定噪声底，相邻同音占用探测窗或尾音不可测时保守保留。普通和弦音程不会进入整数泛音候选。由此保护强真实八度、带独立攻击的弱同时八度/十二度/和弦（包括时值短于同时低音的独立音）、弱但有独立起音的跨手音、单槽和弦、稳定平行声部与持续独立声部，同时删除纯三次自然泛音。上一轮 `harmonic-evidence-v3` 固定产物、指纹、测试数和离线包因 fresh Stage 1 审查发现通用清理/噪声/相邻同音 HIGH 而失效；修复后必须全部重新生成和复核，不能沿用上一轮数值。
