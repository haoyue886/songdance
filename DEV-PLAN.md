...
| Phase 24 | 计划中 | 实时谱面拖选预览、连续命中、反向/跨系统手势与边缘自动滚动 |
...
**交接顺序：** 后续开发 Agent 先完成 Phase 9 和 Phase 11–16 的钢琴质量闭环，再完成结果工作台 Phase 23 与实时拖选 Phase 24，之后严格按 Phase 17 → 18 → 19 → 20 → 21 → 22 推进多乐器。每个乐器过门禁后才能在选择器中启用；不得为了等完整混音而阻塞已通过的单乐器能力。每个 Phase 单独提交，不允许把模型切换、数据库迁移、谱面算法和 UI 改动混进一个不可归因的提交。
