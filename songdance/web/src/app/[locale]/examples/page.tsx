...
export default async function ExamplesPage({ params }: PageProps) {
  ...
  const t = await getTranslations({ locale, namespace: "examples" });
  return <><SiteHeader /><main className="mx-auto w-full max-w-7xl px-5 pb-20 pt-8 sm:px-8 lg:px-10"><section className="mb-10 border-b border-[#d9e3dd] pb-8"><p className="text-xs font-bold uppercase text-[#147d70]">{t("eyebrow")}</p><h1 className="mt-3 font-display text-4xl font-bold leading-tight sm:text-5xl">{t("heading")}</h1><p className="mt-4 max-w-3xl leading-7 text-[#61716c]">{t("intro")}</p><div className="mt-5 text-sm font-semibold"><Link className="text-[#075e55] underline underline-offset-4" href="/transcribe">{t("cta")}</Link></div></section><ExampleClient /></main><SiteFooter /></>;
}
