...
export function ScoreToolbar({ activeMeasure, measureCount, currentTime, duration,
  ...
  return (
    <div className="border-b border-[#d9e3dd] bg-[#f7faf8] px-4 py-3">
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs font-semibold text-[#52635f]">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="whitespace-nowrap tabular-nums" aria-live="polite">
            {measureCount > 0 ? `第 ${activeMeasure + 1} / ${measureCount} 小节` : "小节定位不可用"}
          </span>
        </div>
        <div className="flex items-center gap-1">
          <MeasureButton label="定位当前高亮" disabled={measureCount === 0} onClick={onLocate}>
            <span className="text-[10px] font-bold">定位</span>
          </MeasureButton>
          <MeasureButton label="上一小节" disabled={measureCount === 0 || activeMeasure === 0} onClick={onPrevious}>
            <ChevronLeft size={16} />
          </MeasureButton>
          <MeasureButton label="下一小节" disabled={measureCount === 0 || activeMeasure >= measureCount - 1} onClick={onNext}>
      ...
      <div role="progressbar" aria-label="五线谱播放位置" aria-valuemin={0}
